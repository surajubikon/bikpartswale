...
const ProductDisplayPage = () => {
  ...
  const fetchProductDetails = async () => {
    setLoading(true);
    try {
      // const response = await Axios.post('http://localhost:5173/api/product/get-product-details', { productId });
      const response = await Axios.post('http://localhost:5173/api/product/get-product-details', { productId });
  ...
  const handleRatingSubmit = async () => {
    if (newRating > 0 && newComment.trim() !== "" && username.trim() !== "") {
      try {
        ...
        const response = await Axios.post('http://localhost:5173/api/product/add-rating-comment', {
  ...
  const handleRatingDelete = async (ratingId) => {
    try {
      const response = await Axios.delete('http://localhost:5173/api/product/delete-rating-comment', { data: { ratingId } });
      // const response = await Axios.delete('http://localhost:5173/api/product/delete-rating-comment', { data: { ratingId } });
      const { data: responseData } = response;
      if (responseData.success) {
